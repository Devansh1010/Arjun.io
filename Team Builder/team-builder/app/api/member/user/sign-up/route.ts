...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!username || !email || !password) {
      return createResponse({ success: false, message: 'Missing fields' }, StatusCode.BAD_REQUEST)
    }
    ...
    if (existingUser) {
      ...
      if (!userDoc) {
        ...
        return createResponse({ success: false, message: 'User not updated' }, StatusCode.CONFLICT)
      }
    }
